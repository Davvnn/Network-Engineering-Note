...
1\. TCP 또는 UDP 통신이 실패하면 단말의 Source IP Address, Destination IP Address, Source Port 및 Destination Port가 올바른지 확인한다.

2\. Wireshark를 실행한 후 목적지 서비스에 접속하여 해당 통신의 Packet을 Capture한다.

3\. TCP 통신이라면 TCP Flag를 확인하여 3-Way Handshake가 정상적으로 진행되는지 확인한다.
- 출발지 → 목적지: SYN
- 목적지 → 출발지: SYN-ACK
- 출발지 → 목적지: ACK

4\. UDP 통신이라면 목적지 IP Address와 UDP Port를 기준으로 UDP Datagram이 정상적으로 전송되는지 확인한다.

5\. 출발지에서 Packet이 전송되었지만 목적지에서 확인되지 않으면 중간 네트워크 구간과 방화벽을 확인한다.
- 방화벽 Log에서 해당 TCP 또는 UDP 통신이 차단되었는지 확인한다.

6\. 목적지에서 Packet을 수신했지만 응답하지 않으면 목적지 Application의 상태를 확인한다.

7\. 목적지 Application이 응답을 전송했지만 출발지에서 수신하지 못하면 Return Path와 방화벽을 확인한다.
